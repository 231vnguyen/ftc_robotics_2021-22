...
@Disabled
@TeleOp(name = "TeleOpBackup", group = "Testing")



public class TeleOpModeBackup extends LinearOpMode {
        ...
        while (opModeIsActive()) {
            ...
            switch (autointakeState) {
                ...
                case SHARED_LEFT:
                    ...
                    if (autoIntakeTime.seconds() > .5 && autoIntakeTime.seconds() < .75) {
                        verticalSlide.setTargetPosition((int) slideValues[1]);
                    }

                    if (autoIntakeTime.seconds() > .75) {
